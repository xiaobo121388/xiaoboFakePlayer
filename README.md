...
- 即时动作：看向、旋转、移动、导航、跳跃、停止、传送、攻击、挖掘、使用物品及方块/实体交互。
- 集中式自动行为：跟随、攻击、挖掘、按视线或指定坐标放置方块，以及定时使用物品；每 tick 有公平调度和方块读取预算。
...
动作命令还包括 `fp_lookat`、`fp_navigate`、`fp_move`、`fp_rotate`、`fp_setrotation`、`fp_tp_here`、`fp_jump`、`fp_stop`、`fp_useitem`、`fp_break`、`fp_interactblock`、`fp_useitemonblock`、`fp_attack`、`fp_interactentity`、`fp_lookatentity` 和 `fp_follow`。坐标、距离、维度、视线、区块加载与真实库存槽会在应用层统一校验。
...
| 自动放置方块 | 已实现，需 1.26.33 实机验收 | 从指定库存槽对视线命中面或目标坐标的相邻支撑面执行真实放置 |
...
6. 自动放置分别验证面前视线与指定坐标模式；确认只点击 6 格内可见支撑面、目标格被占时不操作、生存模式成功后只消耗一个指定槽方块。
7. 空手鼠标右键和触屏长按在线假人都只打开一次对应设置界面；手持物品、普通玩家和无权限玩家不会绕过既有行为或权限。
8. 假人安全下线后，分别验证主手、副手、36 格背包、盔甲与副手交换，以及保留假人的物品/经验回收；确认附魔、耐久、名称、Lore 和容器物品不丢失，事务冲突可从恢复中心继续。
