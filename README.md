...
- 集中式自动行为：跟随、攻击、挖掘、按视线或指定坐标放置方块，以及定时使用物品；攻击、挖掘、放置和定时使用互斥，启用其中一项会自动关闭其余三项，跟随可独立运行；每 tick 有公平调度和方块读取预算。
...
| 自动放置方块 | 已实现，需 1.26.33 实机验收 | 普通支撑使用模拟玩家 API；面前模式仅在潜行命中箱子时直接放入空气目标格，坐标模式遇到箱子支撑时直接放置，并同步指定槽库存 |
...
6. 自动放置分别验证面前视线与指定坐标模式；两种模式对普通支撑使用模拟玩家 API，面前模式仅在假人潜行命中箱子且目标为空气时直接设块，坐标模式遇到箱子支撑且目标为空气时直接设块；面前模式只处理 6 格内的视线命中，目标格被占时不操作，生存模式成功后只消耗一个指定槽方块，创造模式不消耗。
