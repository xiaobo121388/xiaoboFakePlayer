...
当前 schema：catalog 3、permissions 1、operations 2。catalog schema 1/2 在内存迁移，旧记录获得默认行为配置和默认皮肤；既有 schema 3 记录缺少新增的 `place` 行为子配置时，会补为默认关闭状态，只有旧版 `slot` 字段时则迁移为指定槽位模式。攻击、挖掘、自动交互（放置）和定时使用是互斥动作行为；历史记录若同时启用多项，加载时按攻击、挖掘、自动交互（放置）、定时使用的顺序只保留第一项，再通过正常提交路径持久化。
