...
| 自动放置方块 | 已实现，需 1.26.33 实机验收 | 普通支撑使用模拟玩家 API；坐标模式遇到箱子支撑时直接放入空气目标格并同步指定槽库存 |
...
6. 自动放置分别验证面前视线与指定坐标模式；面前模式只点击 6 格内命中的支撑面，指定坐标模式对普通支撑使用模拟玩家 API，对箱子支撑在目标为空气时直接设块；目标格被占时不操作，生存模式成功后只消耗一个指定槽方块，创造模式不消耗。
