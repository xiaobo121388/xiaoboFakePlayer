...
- 即时动作：看向、旋转、移动、导航、跳跃、潜行、站立、停止、传送、攻击、挖掘、使用物品及方块/实体交互；“与生物交互”会列出假人周围 10 格内的全部生物，也可输入生物 ID 与最近的同类目标交互。
...
| 即时生物交互 | 已实现，需 1.26.33 实机验收 | 使用 `Dimension.getEntities()` 枚举假人周围 10 格内的生物并按距离排序；枚举能力关闭时回退为实体类型 ID 输入，再调用 `SimulatedPlayer.interactWithEntity()` 与最近同类目标交互 |
...
10. 在即时动作中打开“与生物交互”，确认假人周围 10 格内的生物全部按距离显示；分别点击近处和远处目标，并输入 `minecraft:cow` 等实体类型 ID，确认选择最近同类目标、目标消失或被遮挡时给出明确结果，交互可能改变的主手物品会触发后续库存检查点。
