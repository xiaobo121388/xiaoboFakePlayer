import { CommandPermissionLevel, CustomCommandParamType, CustomCommandStatus, Entity, system, world, } from "@minecraft/server";
...
export function registerCommands(registry, services) {
    ...
    registry.registerCommand({
        ...
        if (!pending.ok) {
            ...
            pending.value.forEach((operation) => lines.push(`${operation.id}: ${operation.kind}/${operation.phase} fake=${operation.fakePlayerId} player=${operation.playerId}`));
        }
        lines.push("capabilities:");
        CAPABILITY_MATRIX.capabilities.forEach((capability) => lines.push(`${capability.id}=${capability.availability}/${capability.verification}`));
    ...
    registry.registerCommand({
        ...
        const result = services.lifecycle.bringOnline(actor, record.id, record.recordRevision, locationMode === "here" ? playerLocation(player) : undefined);
        player.sendMessage(result.ok
    ...
    registry.registerCommand({
        ...
        const result = services.lifecycle.setRespawnRule(actor, record.id, record.recordRevision, mode, mode === "manual" ? playerLocation(player) : undefined);
        player.sendMessage(result.ok
...
function registerBehaviorCommands(registry, services) {
...
function registerCoordinateAction(registry, services, name, description, kind, withSpeed = false) {
    registry.registerCommand({
        name: `xiaobo:${name}`,
        description,
        permissionLevel: CommandPermissionLevel.Any,
        ...
        const action = kind === "look_at"
            ? { kind, dimension: player.dimension.id, position }
            : { kind, dimension: player.dimension.id, position, speed };
        performForRecord(services, player, actor, args[0], action);
    }));
}
function registerSimpleAction(registry, services, name, description, action) {
    registry.registerCommand({
        name: `xiaobo:${name}`,
        description,
        permissionLevel: CommandPermissionLevel.Any,
...
function registerBlockAction(registry, services, name, description, kind, withSlot) {
    ...
    registry.registerCommand({
        ...
        const action = kind === "use_item_on_block"
...
function registerEntityAction(registry, services, name, description, kind, withSpeed = false) {
    registry.registerCommand({
        name: `xiaobo:${name}`,
        description,
        permissionLevel: CommandPermissionLevel.Any,
...
function schedulePlayerCommand(services, command, allowWhenNotReady = false) {
    return (origin, ...args) => {
...
function findRecord(service, actor, reference) {
    if (typeof reference !== "string" || reference.trim().length === 0)
        return "必须提供假人 ID 或名称。";
...
function parseRespawnMode(value) {
...
function blockFaceArgument(value) {
    return value === "down" || value === "east" || value === "north"
        || value === "south" || value === "up" || value === "west"
        ? value
        : "up";
}
function entityArgument(value) {
    const targets = Array.isArray(value)
        ? value.filter((target) => target instanceof Entity && target.isValid)
        : [];
