...
import { snapshotId } from "./inventoryService.js";
export class LifecycleService {
    ...
    takeOffline(actor, id, expectedRecordRevision) {
        ...
        try {
            ...
            if (!offline.ok)
                return offline;
            const finalRecord = { ...offline.value, inventoryRevision: snapshotRevision };
    ...
    bringOnline(actor, id, expectedRecordRevision, location) {
        ...
        try {
            ...
            if (pending.value.inventoryRevision !== null) {
                const restored = this.snapshots.restore(id, snapshotId(id, pending.value.inventoryRevision));
                if (!restored.ok)
                    return restored;
            }
            ...
            if (!online.ok)
                return online;
            const committed = commitCatalogRecord(this.stateStore, prepared.value.catalogRevision, prepared.value.catalog, online.value);
            return committed.ok ? ok(committed.value.record) : committed;
        }
        finally {
            lease.value.release();
    ...
    recycle(actor, id, expectedRecordRevision) {
        ...
        const inventoryTransfer = this.inventory.transferItems(actor, id, context.value.record.recordRevision, { kind: "recycle_all" });
        if (!inventoryTransfer.ok)
            return inventoryTransfer;
        let record = inventoryTransfer.value;
        if (record.totalExperience > 0) {
            const experienceTransfer = this.inventory.transferExperience(actor, id, record.recordRevision, record.totalExperience);
            if (!experienceTransfer.ok)
                return experienceTransfer;
    ...
    finishRenaming(context) {
        ...
        if (operation.target === "online") {
            ...
            if (this.runtime.get(context.record.id) === undefined)
                this.runtime.spawn(spawnRequest(context.record));
            if (context.record.inventoryRevision !== null) {
                const restored = this.snapshots.restore(context.record.id, snapshotId(context.record.id, context.record.inventoryRevision));
                if (!restored.ok)
                    return restored;
            }
        }
