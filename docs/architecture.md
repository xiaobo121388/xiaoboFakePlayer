...
- 默认删除：只允许 offline；先完成全部物品和经验事务，再进入 deleting、删除快照、删除 record。局部恢复失败的 `error` 记录没有可无损回收的权威快照，仅允许 OP 明确放弃内容后进入 deleting。
- 启动恢复被单个非稳定记录阻断时，OP 可在修复界面明确放弃该假人的全部数据。该路径拒绝稳定 online/offline 记录和存在待恢复事务的记录，先持久化 deleting 意图，再幂等断开实体、删除已知快照和 record。对于 catalog 中没有记录但实体带有唯一稳定标签的孤儿，只允许 OP 在确认无关联事务且重读 catalog 后断开该明确实体，不猜测快照 revision。每次处理后都重新运行完整启动恢复；只有恢复成功才解除只读隔离。
- 复活：不恢复死亡前快照；先复活，再对复活后的真实背包建立新权威快照，避免与死亡掉落复制。

所有恢复函数必须幂等。未知 SAPI 异常在边界添加操作和稳定 ID 上下文后继续抛出，由命令、Forms、启动恢复或 tick 边界统一记录。

稳定 `online` 和 `offline` 记录都可进入背包管理。在线概览直接读取活体 41 槽；在线写操作先建立即时检查点，再复用与离线操作相同的可恢复事务，并将已验证的 after image 写回活体。周期调度每 2 tick 错峰处理一个假人；常规完整检查点间隔为 600 tick，在线库存变更标记为 dirty 后在距上次检查点至少 100 tick 时优先保存，避免频繁轮换使当前和上一代都停留在尚未稳定落盘的短窗口。世界 tick 回退时只重建一次时间基准。Forms 在 `show()` 返回后重新获取授权和当前 record revision，不能提交打开表单前缓存的 revision。
...
临时工作区必须先保存原方块结构并写 operations 日志，之后才能放置木桶。新快照通过重新加载并逐槽比较后才可成为 catalog 权威 revision，最后恢复原方块并清理工作区。每个脚本会话第一次进入检查点、安全下线或中断下线恢复路径时，按结构管理器的真实可见性把当前 revision（否则 fallback revision）钉为该会话恢复基线；后续周期检查点和安全下线只替换当前快照并保留该基线。下一次脚本会话若新当前快照仍可见，才将其提升为新会话基线并淘汰更旧结构。启动恢复以 `restore` 的实际结果为准，若当前快照因进程强退未落盘且返回 `NOT_FOUND`，只允许尝试 catalog 明确登记的会话基线，成功后立即清空检查点时间基准以便补拍。预检查不能替代真实恢复结果，其他恢复错误也不能触发回退。物品转移和死亡复活会清除旧回退代，禁止跨所有权变化或死亡掉落回退。
...
7. 按稳定 ID 顺序恢复生命周期中间状态和期望在线记录；无 pending 事务、无残留运行时实体的单记录 `NOT_FOUND` 会提交为 `error` 并继续，其余记录照常恢复。
8. 成功后启动检查点和自动行为调度。

`mixed/conflict` 可以作为可诊断 pending 保留。对应假人必须保持 `offline`，或保持具有存活实例的 `online`；后者在 pending 清除前暂停周期检查点和自动行为。单记录快照缺失的 `error` 会保留原始错误和中断 operation 供诊断，健康记录与新建继续可用；只允许 OP 明确彻底删除，不允许用空库存冒充恢复。catalog/permissions/operations 损坏、结构工作区失败、孤立稳定标签实体、pending 库存事务精确快照缺失及其他跨记录不变量错误仍会阻止 ready。
