...
export type ToolKind = "axe" | "hoe" | "pickaxe" | "shears" | "shovel";

const BASE_MELEE_DAMAGE: Readonly<Record<string, number>> = {
...
export function selectBestWeaponSlot(
    items: readonly SelectableInventoryItem[],
    selectedSlot: number,
): number | undefined {
    let best: SelectableInventoryItem | undefined;
    let bestDamage = -1;
    for (const item of items) {
        const baseDamage = BASE_MELEE_DAMAGE[item.typeId];
        if (baseDamage === undefined || exhausted(item)) continue;
...
export function toolKind(typeId: string): ToolKind | undefined {
...
export function selectReplacementToolSlot(
    items: readonly SelectableInventoryItem[],
    kind: ToolKind,
    selectedSlot: number,
): number | undefined {
    let best: SelectableInventoryItem | undefined;
    for (const item of items) {
        if (item.slot === selectedSlot || toolKind(item.typeId) !== kind || exhausted(item)) continue;
        if (best === undefined
...
function exhausted(item: SelectableInventoryItem): boolean {
    return item.remainingDurability !== null && item.remainingDurability <= 0;
}

function durabilityRank(item: SelectableInventoryItem): number {
