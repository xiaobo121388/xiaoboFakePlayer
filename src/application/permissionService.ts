import type { PermissionGrant } from "../domain/model.js";
import { isAllowed, type ActorIdentity } from "../domain/permissions.js";
import { err, ok, type Result } from "../domain/results.js";
import type { WorldStateStore } from "./ports.js";

export type GrantKind = "can_place" | "can_set";
...
export class PermissionService {
    public constructor(private readonly stateStore: WorldStateStore) {}

    public capabilities(actor: ActorIdentity): Result<PermissionCapabilities> {
    ...
    public setGrant(
        ...
        if (!actor.isOperator) return err("PERMISSION_DENIED", "只有 OP 可以修改假人权限。");
        if (target.playerId.length === 0) return err("INVALID_STATE", "目标玩家没有可用的 PlayFab 稳定 ID。");
        const loaded = this.stateStore.loadPermissions();
        if (!loaded.ok) return err("CONFLICT", loaded.diagnostics.join("; "));
    ...
    public list(actor: ActorIdentity): Result<readonly PermissionGrant[]> {
        if (!actor.isOperator) return err("PERMISSION_DENIED", "只有 OP 可以查看权限列表。");
        const loaded = this.stateStore.loadPermissions();
        if (!loaded.ok) return err("CONFLICT", loaded.diagnostics.join("; "));
