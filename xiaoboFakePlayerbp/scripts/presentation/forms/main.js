...
async function openEntityInteractionForm(player, services, record) {
    await formBoundary(player, `interact-entity:${record.id}`, async () => {
        ...
        if (!current.ok)
            return sendError(player, current.error.message);
        const targets = services.behavior.listInteractionTargets(actorIdentity(player), current.value.id, current.value.recordRevision);
        if (!targets.ok)
...
async function performEntityInteraction(player, services, id, targetId) {
    ...
    if (!current.ok)
        return sendError(player, current.error.message);
    const result = services.behavior.perform(actorIdentity(player), current.value.id, current.value.recordRevision, { kind: "interact_entity", targetId });
    if (!result.ok)
        return sendError(player, result.error.message);
    player.sendMessage({ translate: "xiaobo.fp.message.action_ok", with: [current.value.name] });
}
...
async function openOtherSettingsForm(player, services, record) {
    const response = await new ModalFormData()
        .title(t("xiaobo.fp.form.other_settings.title"))
        .dropdown(t("xiaobo.fp.form.other_settings.game_mode"), GAME_MODES.map((mode) => t(`xiaobo.fp.gamemode.${mode}`)), {
        defaultValueIndex: Math.max(0, GAME_MODES.indexOf(record.gameMode)),
    })
        .dropdown(t("xiaobo.fp.form.other_settings.respawn_mode"), RESPAWN_MODES.map((mode) => t(`xiaobo.fp.respawn.${mode}`)), {
    ...
    await runRecordMutation(player, services, record.id, (current) => services.lifecycle.setOtherSettings(actorIdentity(player), current.id, current.recordRevision, {
        gameMode,
        keepSaturated,
        respawnMode,
...
async function confirmRepairDiscard(player, services, record) {
    const response = await new MessageFormData()
        .title(t("xiaobo.fp.form.delete.title"))
        .body({ translate: "xiaobo.fp.form.delete.body", with: [record.name, record.id] })
        .button1(t("xiaobo.fp.form.cancel"))
        .button2(t("xiaobo.fp.form.delete.confirm"))
        .show(player);
    if (response.canceled || response.selection !== 1 || !player.isValid)
        return;
    const candidates = services.lifecycle.listRepairCandidates(actorIdentity(player));
    if (!candidates.ok)
        return sendError(player, candidates.error.message);
    const current = candidates.value.find((candidate) => candidate.id === record.id);
    if (current === undefined)
        return sendError(player, `假人 ${record.id} 已不再需要人工重置。`);
    const result = services.lifecycle.discardBroken(actorIdentity(player), current.id, current.recordRevision);
    if (!result.ok)
        return sendError(player, result.error.message);
    player.sendMessage({ translate: "xiaobo.fp.message.deleted", with: [result.value.name] });
    await retryRecoveryAndOpenNext(player, services);
}
async function confirmOrphanDiscard(player, services, orphan) {
    const response = await new MessageFormData()
        .title(t("xiaobo.fp.form.delete.title"))
    ...
    if (current === undefined)
        return sendError(player, `孤儿假人实体 ${orphan.id} 已不再需要人工重置。`);
...
function loadOrphanRepairCandidates(services, actor) {
    try {
...
async function retryRecoveryAndOpenNext(player, services) {
