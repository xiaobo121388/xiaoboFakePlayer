...
- 集中式自动行为：跟随、攻击、挖掘、按视线或指定坐标自动交互（放置）、定时使用物品，以及每 5 秒将指定半径内箭和投掷三叉戟的投掷者改为假人的“三叉戟、箭挂机”；后者默认半径为 20 格，可独立开关且不生成投射物。自动交互可指定快捷栏槽位，也可按物品 ID 在每次动作时遍历 36 格背包，物品 ID 留空表示空手；攻击、挖掘、自动交互和定时使用互斥，启用其中一项会自动关闭其余三项，跟随和三叉戟、箭挂机可独立运行；每 tick 有公平调度和方块读取预算。一次性导航优先于所有自动寻路；跟随与攻击追击或挖掘接近同时启用时，仅在假人超过“跟随停止距离”后由跟随寻路接管，进入该距离后恢复其他寻路。
