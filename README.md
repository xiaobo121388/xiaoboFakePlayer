...
- 保存维度、位置、旋转、游戏模式、持续饱和、选中槽、总经验、复活规则、皮肤、行为配置和期望在线状态。
...
- 即时动作：看向、旋转、移动、导航、跳跃、潜行、站立、停止、传送、攻击、挖掘、使用物品及方块/实体交互；“与实体交互”会列出假人周围 10 格内除自身外的全部实体，也可输入实体 ID 与最近的同类目标交互。
- 集中式自动行为：跟随、攻击、挖掘、按视线或指定坐标自动交互（放置）、定时使用物品，以及每 5 秒将指定半径内箭和投掷三叉戟的投掷者改为假人的“三叉戟、箭挂机”；后者默认半径为 20 格，可独立开关且不生成投射物。自动交互可指定快捷栏槽位，也可按物品 ID 在每次动作时遍历 36 格背包，物品 ID 留空表示空手；指定物品表单可在本次保存时读取玩家当前主手并忽略输入的 ID；攻击、挖掘、自动交互和定时使用互斥，启用其中一项会自动关闭其余三项，跟随和三叉戟、箭挂机可独立运行；每 tick 有公平调度和方块读取预算。一次性导航优先于所有自动寻路；跟随与攻击追击或挖掘接近同时启用时，仅在假人超过“跟随停止距离”后由跟随寻路接管，进入该距离后恢复其他寻路。
- 默认皮肤与在线真人 Persona 外观复制。Persona 数据会随记录保存，并在重新上线时恢复。
- 详情页“其他功能”集中设置生存、创造、冒险或旁观模式、复活规则和持续饱和；在线假人即时切换模式，持续饱和每 5 秒续加状态效果，设置随记录保存。
...
在线假人每 2 tick 错峰检查一个；常规完整检查点间隔为 600 tick（约 30 秒），攻击、放置、交互、使用或挖掘完成等可能改变库存的动作会标记 dirty，并在距上次检查点至少 100 tick（约 5 秒）后优先保存。本次脚本会话第一次检查点、安全下线或中断下线恢复时，会把结构管理器中真实可见的当前代（否则上一代）钉为恢复基线；同一会话后续检查点只轮换当前代，不删除该基线。下次脚本会话只有在较新的当前代仍真实可见时才推进基线。启动恢复以结构管理器的实际恢复结果为准；当前代实际缺失时只回退到 catalog 登记的会话基线。生命周期、Forms 转移和删除事务具有可恢复保证；如果游戏进程在脚本来不及运行时被强杀，在线实体刚发生的外部背包变化最多只能恢复到最近一次已验证且已落盘的检查点。这是公开 Script API 的一致性边界，应在重要操作前正常退出世界并保留世界备份。

若某个没有待恢复事务的假人因当前代和会话基线都缺失而无法恢复，系统会把该记录持久化为 `error`，断开未恢复实体并继续进入 ready；健康假人和新建功能不受影响。详情页、恢复中心和 `/xiaobo:fp_diagnose` 会显示隔离原因；只有 OP 可在明确放弃该记录内容后彻底删除，系统不会伪造空库存或提供“无损恢复”。损坏 bank、孤立稳定标签实体、pending 库存事务的精确快照缺失、非法跨记录组合或无法判定的物品冲突仍会阻止所有普通写操作。
...
| 三叉戟、箭挂机 | 已实现，需 1.26.33 实机验收 | 每 100 tick 查询指定半径内的 `minecraft:arrow` 和 `minecraft:thrown_trident`，通过公开 `minecraft:projectile` 组件将 owner 改为假人；默认 20 格 |
| 空手交互打开设置 | 已实现，需 1.26.33 实机验收 | 使用稳定的 `playerInteractWithEntity` 事件；长按重复事件只打开一个界面 |
| 即时实体交互 | 已实现，需 1.26.33 实机验收 | 使用 `Dimension.getEntities()` 枚举假人周围 10 格内除自身外的全部实体并按距离排序；枚举能力关闭时回退为实体类型 ID 输入，再调用 `SimulatedPlayer.interactWithEntity()` 与最近同类目标交互 |
...
| 持续饱和效果 | 已实现，需 1.26.33 实机验收 | 通过公开状态效果 API 每 5 秒续加 `minecraft:saturation`，关闭时立即移除 |
...
3. 在同一会话完成多次检查点后，以及快照和转移各阶段强制退出，确认启动可见基线、原方块或事务 image 至少有一个权威副本；人为移除单个无事务假人的两代快照时，确认该记录进入 `error`，健康假人和新建仍可用，且只有 OP 能明确彻底删除该记录。
...
10. 在即时动作中打开“与实体交互”，确认假人周围 10 格内除自身外的真人玩家、其他假人、生物、掉落物、盔甲架和载具等实体全部按距离显示；分别点击近处和远处目标，并输入 `minecraft:player`、`minecraft:item` 或 `minecraft:cow` 等实体类型 ID，确认选择最近同类目标、目标消失或被遮挡时给出明确结果，交互可能改变的主手物品会触发后续库存检查点。
11. 分别在假人在线和安全下线时通过“其他功能”切换生存、创造、冒险和旁观模式，验证上线后模式与记录一致；逐一验证手动、死亡位置和玩家出生点三种复活规则；开启持续饱和后等待超过 5 秒确认效果持续，关闭后确认效果立即移除，重进世界后设置仍保留。
12. 开启“三叉戟、箭挂机”，分别使用默认 20 格和自定义半径，在范围内外放置或射出箭与三叉戟；等待至少 5 秒后通过命中伤害归属、忠诚三叉戟返回目标等可观察结果确认范围内投射物 owner 已改为该假人，范围外投射物不变，关闭开关后不再认领。
