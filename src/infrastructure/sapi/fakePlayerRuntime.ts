...
import {
    ...
    type ToolKind,
} from "../../domain/inventorySelection.js";
...
export class SapiFakePlayerRuntime implements FakePlayerRuntime {
    ...
    private readonly miningTools = new Map<FakePlayerId, { readonly slot: number; readonly kind: ToolKind }>();
    ...
    public listTagged(rebind = true): readonly RuntimeFakePlayer[] {
    ...
    private equipBestWeapon(id: FakePlayerId, player: SimulatedPlayer): boolean {
        ...
        if (weaponSlot === undefined || weaponSlot === selectedSlot) return false;
        container.swapItems(weaponSlot, selectedSlot, container);
        return true;
    }

    private replaceExhaustedTool(id: FakePlayerId, player: SimulatedPlayer): boolean {
        ...
        const replacementSlot = selectReplacementToolSlot(
            selectableItems(container),
            replacementKind,
            selectedSlot,
        );
        if (replacementSlot === undefined) return false;
...
function inventoryContainer(
    ...
    operation: "attack" | "mine",
): EntityInventoryComponent["container"] {
    const inventory = player.getComponent(EntityComponentTypes.Inventory) as EntityInventoryComponent | undefined;
    if (inventory?.container === undefined) throw new Error(`${operation} ${id}: 缺少库存容器。`);
    return inventory.container;
}

function selectableItems(container: EntityInventoryComponent["container"]): readonly SelectableInventoryItem[] {
    const items: SelectableInventoryItem[] = [];
    for (let slot = 0; slot < INVENTORY_SLOT_COUNT; slot += 1) {
        const item = container.getItem(slot);
        if (item !== undefined) items.push(selectableItem(slot, item));
    }
    return items;
}

function selectableItem(slot: number, item: ItemStack): SelectableInventoryItem {
...
function isExhausted(item: SelectableInventoryItem): boolean {
