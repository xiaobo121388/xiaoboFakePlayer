...
- 集中式自动行为：跟随、攻击、挖掘、按视线或指定坐标自动交互（放置），以及定时使用物品；自动交互可指定快捷栏槽位，也可按物品 ID 在每次动作时遍历 36 格背包，物品 ID 留空表示空手；指定物品表单可在本次保存时读取玩家当前主手并忽略输入的 ID；攻击、挖掘、自动交互和定时使用互斥，启用其中一项会自动关闭其余三项，跟随可独立运行；每 tick 有公平调度和方块读取预算。一次性导航优先于所有自动寻路；跟随与攻击追击或挖掘接近同时启用时，仅在假人超过“跟随停止距离”后由跟随寻路接管，进入该距离后恢复其他寻路。
...
在线假人每 2 tick 错峰检查一个；常规完整检查点间隔为 600 tick（约 30 秒），攻击、放置、交互、使用或挖掘完成等可能改变库存的动作会标记 dirty，并在距上次检查点至少 100 tick（约 5 秒）后优先保存。检查点保留当前和上一代，启动恢复以结构管理器的实际恢复结果为准；当前代实际缺失时回退到 catalog 登记的上一代。生命周期、Forms 转移和删除事务具有可恢复保证；如果游戏进程在脚本来不及运行时被强杀，在线实体刚发生的外部背包变化最多只能恢复到最近一次已验证且已落盘的检查点。这是公开 Script API 的一致性边界，应在重要操作前正常退出世界并保留世界备份。
...
| 自动交互（放置） | 已实现，需 1.26.33 实机验收 | 普通固体支撑上的方块物品使用 `startBuild()`/`stopBuild()` 原生建造动作，按钮、拉杆、门、非潜行箱子等非固体命中块使用 `interact()` 通用交互；指定物品会从 36 格背包动态查找并临时换入，动作后恢复原槽位；潜行箱子兼容路径仍直接放置并同步真实库存 |
...
6. 自动交互（放置）分别验证面前视线与指定坐标模式，并验证指定快捷栏、指定物品和空手三种来源；方块物品应临时换入槽位 0 后调用 `startBuild()`/`stopBuild()` 并原样恢复，空手或非方块物品应通过一次性瞄准后调用 `interact()`，可激活按钮、拉杆、门等非固体方块。指定物品表单的主手开关应默认关闭，开启后本次保存忽略输入 ID 并记录玩家当前主手，下次打开应显示该 ID 且开关恢复关闭。移动指定物品到其他背包槽后应在下一次动作重新找到并在动作后原样换回；物品不存在时不交互，留空时应临时清空当前快捷栏完成交互并原样恢复。面前模式仅在假人潜行命中箱子、所选物品为方块且目标为空气时直接设块，坐标模式遇到箱子支撑、所选物品为方块且目标为空气时直接设块；面前模式只处理 6 格内的视线命中，方块放置目标格被占时不操作，生存模式成功后只消耗一个实际命中槽的方块，创造模式不消耗。
