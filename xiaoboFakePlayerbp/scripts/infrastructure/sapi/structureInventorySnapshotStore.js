import { BlockComponentTypes, EntityComponentTypes, EquipmentSlot, StructureSaveMode, world, } from "@minecraft/server";
...
const EQUIPMENT_BY_LOGICAL_SLOT = {
...
export class StructureInventorySnapshotStore {
    stateStore;
    runtime;
    constructor(stateStore, runtime) {
        this.stateStore = stateStore;
        this.runtime = runtime;
    }
    save(fakePlayerId, revision) {
        ...
        return this.withWorkspace(operationId, player.dimension, player.location, (workspace) => {
            ...
            world.structureManager.createFromWorld(structureId, workspace.dimension, workspace.operation.origin, offset(workspace.operation.origin, 1, 0, 0), { includeBlocks: true, includeEntities: false, saveMode: StructureSaveMode.World });
    ...
    restore(fakePlayerId, structureId) {
        const player = this.runtime.getHandle(fakePlayerId);
        if (player === undefined)
            return err("INVALID_STATE", `假人 ${fakePlayerId} 没有在线运行时实例。`);
        if (!this.has(structureId))
            return err("NOT_FOUND", `库存快照 ${structureId} 不存在。`);
    ...
    saveImage(structureId, dimension, near, image) {
    ...
    loadImage(structureId, dimension, near) {
        if (!this.has(structureId))
            return err("NOT_FOUND", `结构 image ${structureId} 不存在。`);
    ...
    withWorkspace(operationId, dimension, near, work) {
        ...
        try {
            ...
            if (!cleanup.ok)
                throw new Error(`${operationId}: 操作失败且工作区恢复失败：${cleanup.error.message}`, { cause });
            throw new Error(`${operationId}: 结构库存操作失败。`, { cause });
        }
        const cleanup = this.restoreWorkspace(prepared.value.operation);
        return cleanup.ok ? result : cleanup;
    }
    prepareWorkspace(operationId, dimension, near) {
        ...
        if (existing !== undefined)
            world.structureManager.delete(existing);
        world.structureManager.createFromWorld(backupStructureId, dimension, origin, offset(origin, 1, 0, 0), { includeBlocks: true, includeEntities: false, saveMode: StructureSaveMode.World });
        const operation = {
            id: operationId,
            phase: "prepared",
    ...
    updateWorkspacePhase(id, phase) {
        const loaded = this.stateStore.loadOperations();
        if (!loaded.ok)
            return err("CONFLICT", loaded.diagnostics.join("; "));
        const operation = loaded.state.value.workspace[id];
        if (operation === undefined)
            return err("NOT_FOUND", `未找到结构工作区 ${id}。`);
    ...
    restoreWorkspace(operation) {
        ...
        if (!loaded.ok)
            return err("CONFLICT", loaded.diagnostics.join("; "));
        if (loaded.state.value.workspace[operation.id] === undefined)
            return ok(undefined);
...
export function writePlayerImage(player, image) {
    const inventory = player.getComponent(EntityComponentTypes.Inventory);
    const equipment = player.getComponent(EntityComponentTypes.Equippable);
    if (inventory === undefined || equipment === undefined) {
        return err("INVALID_STATE", `假人 ${player.name} 缺少库存或装备组件。`);
    ...
    for (let slot = 0; slot < TOTAL_SLOT_COUNT; slot += 1) {
        if (slot < 36)
            inventory.container.setItem(slot, image[slot]);
        else
            equipment.setEquipment(equipmentSlotFor(slot), image[slot]);
    }
    return ok(undefined);
}
function placeEmptyBarrels(dimension, origin) {
...
function getBarrels(dimension, origin) {
    const first = dimension.getBlock(origin);
    const second = dimension.getBlock(offset(origin, 1, 0, 0));
    const firstInventory = first?.getComponent(BlockComponentTypes.Inventory);
    const secondInventory = second?.getComponent(BlockComponentTypes.Inventory);
...
function clearBarrels(barrels) {
    barrels.A.clearAll();
    barrels.B.clearAll();
}
function clearBarrelsIfPresent(dimension, origin) {
...
export function imagesEqual(left, right) {
...
export function itemStacksEqual(left, right) {
    if (left === undefined || right === undefined)
        return left === right;
    if (left.amount !== right.amount || left.typeId !== right.typeId)
        return false;
    if (left.isStackable && right.isStackable)
        return left.isStackableWith(right);
...
function itemStackFingerprint(itemStack, depth) {
    const componentIds = itemStack.getComponents().map((component) => component.typeId).sort();
    if (componentIds.some((componentId) => !OBSERVABLE_ITEM_COMPONENTS.has(componentId)))
        return undefined;
    const inventory = itemStack.getComponent("minecraft:inventory");
    if (inventory !== undefined && depth >= MAX_NESTED_CONTAINER_DEPTH)
        return undefined;
    const nestedItems = [];
    ...
    const enchantments = enchantable?.getEnchantments().map((enchantment) => [
        enchantment.type.id,
        enchantment.level,
    ]).sort(([leftId, leftLevel], [rightId, rightLevel]) => (leftId === rightId ? leftLevel - rightLevel : leftId.localeCompare(rightId))) ?? [];
...
function canonicalizeItemValue(value) {
    if (Array.isArray(value))
        return value.map(canonicalizeItemValue);
...
function equipmentSlotFor(logicalSlot) {
    const equipmentSlot = EQUIPMENT_BY_LOGICAL_SLOT[logicalSlot];
    if (equipmentSlot === undefined)
        throw new RangeError(`无效装备逻辑槽位：${logicalSlot}。`);
    return equipmentSlot;
}
function workspaceOrigin(dimension, near) {
