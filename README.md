...
| 自动交互（放置） | 已实现，需 1.26.33 实机验收 | 使用 `interactWithBlock`；指定快捷栏时先切换选中槽，指定物品时从 36 格背包动态查找并临时换入当前快捷栏，空手时临时清空当前快捷栏；指定物品时可将玩家当前主手保存为物品 ID；方块物品在既有箱子兼容路径中直接放置并同步真实库存 |
...
6. 自动交互（放置）分别验证面前视线与指定坐标模式，并验证指定快捷栏、指定物品和空手三种来源；指定快捷栏时应切换假人的选中槽后调用 `interactWithBlock`。指定物品表单的主手开关应默认关闭，开启后本次保存忽略输入 ID 并记录玩家当前主手，下次打开应显示该 ID 且开关恢复关闭。移动指定物品到其他背包槽后应在下一次动作重新找到，并临时换入当前快捷栏完成交互后原样换回；物品不存在时不交互，留空时应临时清空当前快捷栏完成交互并原样恢复。两种目标模式对普通支撑使用模拟玩家 API；面前模式仅在假人潜行命中箱子、所选物品为方块且目标为空气时直接设块，坐标模式遇到箱子支撑、所选物品为方块且目标为空气时直接设块；面前模式只处理 6 格内的视线命中，目标格被占时不操作，生存模式成功后只消耗一个实际命中槽的方块，创造模式不消耗。
