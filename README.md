...
- 自动攻击会遍历背包与快捷栏 0-35，按原版普通近战伤害和锋利等级选择最高伤害武器；同分时保留当前手持，否则优先较小槽位。目标特攻、状态效果和重锤坠落伤害不参与排序。
- 自动挖掘会记住当前手持工具类别；工具耐久耗尽后，会从背包与快捷栏换入同类别且剩余耐久最高的工具。
- 可复制在线玩家的 Persona 外观；经典皮肤纹理无法复制时会使用默认皮肤。
- 空手对在线假人使用交互键，可直接打开该假人的设置界面。
- 提供 OP 恢复中心；遇到中断的物品操作时，可以继续恢复而不是直接覆盖物品。系统进入只读隔离时，`/xiaobo:fpset` 会直接打开修复界面，可逐个选择异常记录或无记录的孤儿实体，并在二次确认后永久放弃其数据、删除目标，再自动重试启动恢复。
...
| 指令 | 用法 |
|---|---|
| `/xiaobo:fpset` | 打开挂机假人管理界面；系统处于只读隔离时，改为向 OP 打开异常修复界面。 |
...
| `/xiaobo:fp_recovery` | 仅 OP 可用。打开待恢复物品操作与异常假人修复中心。 |
...
- 系统显示"正在恢复"时请等待相关区块加载。显示"只读隔离"时，由 OP 打开 `/xiaobo:fpset` 或 `/xiaobo:fp_recovery`：先重试列出的物品事务；没有关联事务时，可选择一个异常记录，或一个没有 catalog 记录的稳定标签孤儿实体，并二次确认永久放弃其物品、经验、快照和设置。修复后系统会自动重试恢复，成功后重新开放全部功能。
- 如果 catalog 或事务 A/B bank 本身均已损坏，修复界面会显示底层状态不可读且不会猜测删除目标；请保留世界备份并使用 `/xiaobo:fp_diagnose` 记录诊断。
