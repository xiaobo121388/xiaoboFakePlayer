...
- 集中式自动行为：跟随、攻击、挖掘和定时使用物品；每 tick 有公平调度和方块读取预算。
...
6. 空手鼠标右键和触屏长按在线假人都只打开一次对应设置界面；手持物品、普通玩家和无权限玩家不会绕过既有行为或权限。
7. 分别在假人在线和安全下线时验证主手、副手、36 格背包、盔甲与副手交换，以及保留假人的物品/经验回收；确认在线活体立即更新，附魔、耐久、名称、Lore 和容器物品不丢失，强制退出后的事务可从恢复中心继续。
