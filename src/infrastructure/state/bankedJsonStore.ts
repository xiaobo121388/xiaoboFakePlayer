import type { LoadedState, VersionedState } from "../../application/ports.js";
import { err, ok, type Result } from "../../domain/results.js";

export type BankName = "A" | "B";

export interface StringPropertyBackend {
...
export interface StateCodec<T> {
    ...
    decode(schemaVersion: number, payload: unknown): T | undefined;
}
...
interface ValidBank<T> {
    readonly bank: BankName;
    readonly state: VersionedState<T>;
}

const DEFAULT_MAX_PROPERTY_BYTES = 30_000;

export class BankedJsonStore<T> {
    public constructor(
        private readonly backend: StringPropertyBackend,
        private readonly prefix: string,
        private readonly codec: StateCodec<T>,
        private readonly maxPropertyBytes = DEFAULT_MAX_PROPERTY_BYTES,
    ) {}

    public load(): LoadedState<T> {
        ...
        const validBanks = [this.readBank("A", bankAValue), this.readBank("B", bankBValue)].filter(
            (bank): bank is ValidBank<T> => bank !== undefined,
        );
        ...
        const recovered = validBanks.reduce((latest, candidate) =>
            candidate.state.revision > latest.state.revision ? candidate : latest,
        );
    ...
    public commit(expectedRevision: number, value: T): Result<VersionedState<T>> {
        ...
        const target: BankName = active === "A" ? "B" : "A";
    ...
    private readBank(bank: BankName, serialized: string | undefined): ValidBank<T> | undefined {
    ...
    private bankKey(bank: BankName): string {
        return `${this.prefix}:${bank.toLowerCase()}`;
    }
}

function createEnvelope(schemaVersion: number, revision: number, payload: unknown): BankEnvelope {
    return {
        schemaVersion,
        revision,
...
function isEnvelope(value: unknown): value is BankEnvelope {
    if (typeof value !== "object" || value === null) return false;
    const candidate = value as Partial<BankEnvelope>;
...
export function calculateChecksum(schemaVersion: number, revision: number, payload: unknown): string {
