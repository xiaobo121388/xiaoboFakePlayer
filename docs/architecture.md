...
临时工作区必须先保存原方块结构并写 operations 日志，之后才能放置木桶。新快照通过重新加载并逐槽比较后才可成为 catalog 权威 revision，最后恢复原方块并清理工作区。周期检查点保留当前和上一代两个已验证快照；启动恢复以 `restore` 的实际结果为准，若当前快照因进程强退未落盘且返回 `NOT_FOUND`，只允许尝试 catalog 明确登记的上一代，成功后立即清空检查点时间基准以便补拍。预检查不能替代真实恢复结果，其他恢复错误也不能触发回退。物品转移和死亡复活会清除旧回退代，禁止跨所有权变化或死亡掉落回退。
