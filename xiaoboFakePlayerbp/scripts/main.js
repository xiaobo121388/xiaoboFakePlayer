...
system.runInterval(() => {
    ...
    try {
        ...
        if (!result.ok)
            console.error(`[xiaobo-fake-player] auto respawn poll failed: ${result.error.message}`);
    }
    catch (cause) {
        const message = cause instanceof Error ? cause.message : String(cause);
...
system.runInterval(() => {
    ...
    try {
        ...
        if (!result.ok)
            console.error(`[xiaobo-fake-player] saturation refresh failed: ${result.error.message}`);
    }
    catch (cause) {
        const message = cause instanceof Error ? cause.message : String(cause);
