...
export interface OtherSettingsRequest {
    readonly gameMode: FakePlayerGameMode;
    readonly keepSaturated: boolean;
    readonly respawnMode: RespawnMode;
    readonly manualRespawnLocation?: SavedLocation;
}
...
export class LifecycleService {
    ...
    public setOtherSettings(
        ...
        if (context.value.record.lifecycle.kind !== "online" && context.value.record.lifecycle.kind !== "offline") {
            return err(
                "INVALID_STATE",
                `假人 ${id} 当前处于 ${context.value.record.lifecycle.kind}，不能修改其他功能。`,
            );
        }
        ...
        try {
            if (context.value.record.lifecycle.kind === "online") {
                if (request.keepSaturated !== context.value.record.keepSaturated) {
                    ...
                    if (!saturation.accepted) return err("CONFLICT", `无法更新假人 ${id} 的持续饱和状态。`);
            ...
            if (committed.ok) return ok(committed.value.record);
            const rollbackFailures = rollbackRuntime();
            ...
            throw new Error(
                `other settings ${id}: ${message}`
                + (rollbackFailures.length === 0 ? "" : `；无法回滚${rollbackFailures.join("、")}。`),
                { cause },
            );
        } finally {
            lease.value.release();
    ...
    public refreshSaturation(): Result<number> {
        const loaded = this.stateStore.loadCatalog();
        if (!loaded.ok) return err("CONFLICT", loaded.diagnostics.join("; "));
        const rejected: FakePlayerId[] = [];
        let refreshed = 0;
        for (const record of Object.values(loaded.state.value.records)) {
            if (record.lifecycle.kind !== "online" || !record.keepSaturated) continue;
            ...
            try {
                ...
                if (receipt.accepted) refreshed += 1;
